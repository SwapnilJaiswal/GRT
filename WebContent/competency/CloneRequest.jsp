...
function validate_form(){
	//	alert("validating "+attachArr);
		...
		var SNO =<%= oldSno.toString() %>	; 
	//	alert("SNO"+SNO);
		//alert("attachArr"+attachArr.length);
		var arr="";
		...
		//alert("arr"+arr);
		 //document.getElementById("y").value=attachArr[0]+"$$"+attachArr[1];
		 document.getElementById("y").value=arr;
		 document.getElementById("oldSno").value=SNO;
		// document.getElementById("y").value=document.getElementById("y").value=attachArr+"$$"+document.getElementById("y").value
		// alert("test"+document.getElementById("y").value);
...
/**Added for Bug 87170-Cloning a task.Added to download file from DownloadServlet**/

 
function downloadFile(){
	//alert("Inside download file");
	 <%
	 ...
	 {
		 System.out.println("downloadFile SNo is"+SNo);
	 }
		 
	%>
	
	var SNO =<%= SNo.toString() %>	; 
	//alert("SNO"+SNO);
	$("form").attr("action", "DownloadFileServlet?SNo=" + SNO);
	//alert("SNO"+SNO);
 ...
 function load(js_obj)
 {
 	...
 	$(document).ready(
 			function()
 				...
 				attachArr=res;
 				//alert("on load attacharr"+attachArr);
 				//alert('jsonObj.SNo11 '+jsonObj.SNo);
 				<%
 				 String old = request.getParameter( "SNo" );
 				
 					 
 				%>
 				
 				var old =<%= old.toString() %>	; 
 			//	alert("old"+old);
 				
 				
 			//	var s=request.getParameter("SNo");
 				
 				var links = '<a href="download?sno='+old+'&fileName='+encodeURIComponent(res[0])+'">'+res[0]+'</a><br><br>';
 				...
 				{
 					//alert('jsonObj.SNo'+old);
 				 links = links+'<a href="download?sno='+old+'&fileName='+encodeURIComponent(res[0])+'">'+res[i]+'</a><br><br>';
 				}
        ...
        <form name="form" id="form" method="post" action="NewEntry" enctype='multipart/form-data'>
			...
			<table class="classification" id="classification">
				...
				<!--Division 2 Ends-->	
						...
						<tbody>
							...
							<tr>
								...
								<td align="left"><input type="text" name="InitialBugCount"
									id="InitialBugCount" maxlength="4"
					...
					<table class="attachfiles" id="attachfiles">
						<tbody>
						<tr>
						...
						<tr>
							...
							<tr><td><input type="hidden" name="y" value="" id="y">
							</td>
							</tr>
							<tr><td><input type="hidden" name="oldSno" value="" id="oldSno">
							</td>
							</tr>
					...
					<table class="submit" id="submitDiv">
						<tr>
							<td colspan="2">
							<input type="hidden" name="existingHwNew" value="${js_obj_json.ExistingHardwareNew}" />
