 ...
 function load(js_obj)
 {
 	...
 	$(document).ready(
 			function()
				...
				if(existingHwNew == "Y") {
					$("#ExistingHardwareinfo").chosen({search_contains:true}).trigger("chosen:updated");
				}				
 				var attachment = jsonObj.attachments;
 				var res = attachment.split(";");
 				var links = '<a href="download?sno='+jsonObj.SNo+'&fileName='+res[0]+'">'+res[0]+'</a><br><br>';
 				for(var i=1;i<res.length;i++)
 				{
 				 links = links+'<a href="download?sno='+jsonObj.SNo+'&fileName='+res[i]+'">'+res[i]+'</a><br><br>';
 				}
			...
			success: function( json ) {
				...
				if($('#DeliverableClassification').val() == "Deployment Services" || $('#DeliverableClassification').val() == "Design Services" || $('#CapId').val() != ""){
					$('#pidmand').hide();
					$('#taskmand').hide();
				} 
 				}
			...
			<table class="classification" id="classification">
				...
				<!--Division 2 Ends-->	
					...
					<table  id="attachfiles">
						<tbody>
						<tr>
							<th class="theader" align="left" colspan='100%'>Attachments (Max File size 25MB per file )</th>
						</tr>
						<tr>
							...
							<tr><td colspan='100%'><input type="hidden" name="x" value=0 id="x" />
							</td>
							</tr>
											...
											<tr>
												...
												<input type="submit" value="submit"  onclick = "return validate_form();"style="float:right;top:10px;position:relative;"/></td>
												<!--
												<td colspan="2"><input type="button" value="EDIT THIS REQUEST" onclick="makeEditable();"style="float:right;top:10px;position:relative;"/></td>
												!-->
											</tr>
